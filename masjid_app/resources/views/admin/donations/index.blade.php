  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="w-64 bg-white shadow-xl z-20 hidden md:block flex-shrink-0">
      ...
      <nav class="mt-6 px-4 space-y-2 overflow-y-auto h-[calc(100vh-180px)]">
        ...
        <a href="{{ route('admin.donations.index') }}" class="sidebar-item active w-full flex items-center px-4 py-3 text-left rounded-lg text-sm font-medium">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center bg-gray-50">
                    ...
                    <div class="text-xs text-gray-500 bg-white px-3 py-1 rounded border border-gray-200 shadow-sm">
                        Total Rekod: <strong>{{ $donations->total() }}</strong>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-gray-100 bg-white">
                            ...
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-800">{{ $donation->donor->name ?? 'Hamba Allah' }}</div>
                                    <div class="text-xs text-gray-500 italic">
                                        {{ str_contains($donation->donor_type, 'Participant') ? 'Ahli Kariah' : 'AJK/Admin' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    {{-- LOGIK UNTUK TUKAR ID KE NAMA PROGRAM --}}
                                    @php
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    <div class="flex justify-center space-x-2">
                                        <form action="{{ route('admin.donations.verify', $donation->id) }}" method="POST" onsubmit="return confirm('Sahkan penerimaan derma ini?');">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form action="{{ route('admin.donations.reject', $donation->id) }}" method="POST" onsubmit="return confirm('Tolak/Batalkan derma ini?');">
                                            @csrf
                                            @method('PATCH')
