  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="w-64 bg-white shadow-xl z-20 hidden md:block flex-shrink-0">
      ...
      <nav class="mt-6 px-4 space-y-2">
        ...
        <a href="{{ route('admin.events.index') }}" class="sidebar-item active w-full flex items-center px-4 py-3 text-left rounded-lg text-sm font-medium">
    ...
    <div class="flex-1 overflow-y-auto bg-gray-50 h-full">
      ...
      <main class="p-8">
        <div class="fade-in max-w-4xl mx-auto">
            ...
            <form method="POST" action="{{ route('admin.events.store') }}" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                @csrf
                
                <div class="p-8 space-y-6">
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Nama Acara</label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Contoh: Kuliah Maghrib Perdana"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('title') border-red-500 @enderror">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" name="event_date" id="event_date" value="{{ old('event_date') }}" required
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('event_date') border-red-500 @enderror">
                            @error('event_date') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="capacity" class="block text-sm font-medium text-gray-700 mb-1">Kapasiti Peserta</label>
                            <input type="number" name="capacity" id="capacity" value="{{ old('capacity') }}" required min="1" placeholder="Contoh: 100"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('capacity') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <select name="venue" id="venue" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('venue') border-red-500 @enderror">
                            <option value="">Sila pilih lokasi...</option>
                            <option value="Dewan Solat Utama" {{ old('venue') == 'Dewan Solat Utama' ? 'selected' : '' }}>Dewan Solat Utama</option>
                            <option value="Dewan Seminar 1" {{ old('venue') == 'Dewan Seminar 1' ? 'selected' : '' }}>Dewan Seminar 1</option>
                            <option value="Bilik Mesyuarat" {{ old('venue') == 'Bilik Mesyuarat' ? 'selected' : '' }}>Bilik Mesyuarat</option>
                            <option value="Dataran Masjid" {{ old('venue') == 'Dataran Masjid' ? 'selected' : '' }}>Dataran Masjid</option>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="5" required placeholder="Masukkan butiran lanjut mengenai acara ini..."
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                        @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex justify-end space-x-3">
                    <a href="{{ route('admin.events.index') }}" class="px-6 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors shadow-sm">
                        Batal
                    </a>
