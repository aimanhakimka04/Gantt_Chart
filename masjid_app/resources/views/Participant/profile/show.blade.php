            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 sticky top-24">
                    ...
                    <nav class="p-2 space-y-1">
                        ...
                        <a href="{{ route('Participant.profile.show') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg bg-blue-50 text-blue-700">
                            <i class="fas fa-user w-6 text-center mr-2"></i> Profil Saya
                        </a>

                        <a href="{{ route('participant.registrations') }}" class="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-gray-50 hover:text-blue-700 transition-colors">
            ...
            <div class="lg:col-span-3 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6 flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                    <div class="flex-shrink-0">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::guard('participant')->user()->name) }}&background=1e40af&color=fff&size=128&bold=true" 
                             class="w-24 h-24 rounded-full border-4 border-blue-50 shadow-md" 
                             alt="Profile Picture">
                    </div>
                    <div class="text-center md:text-left flex-1">
                        <h2 class="text-2xl font-bold text-gray-800">{{ Auth::guard('participant')->user()->name }}</h2>
                        <div class="mt-1 flex flex-col md:flex-row md:items-center text-gray-500 text-sm space-y-1 md:space-y-0 md:space-x-4">
                            <span class="flex items-center justify-center md:justify-start">
                                <i class="fas fa-envelope mr-2 text-gray-400"></i>{{ Auth::guard('participant')->user()->email }}
                            </span>
                            <span class="flex items-center justify-center md:justify-start">
                                <i class="fas fa-phone mr-2 text-gray-400"></i>{{ Auth::guard('participant')->user()->phone ?? 'Tiada No. Telefon' }}
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <form method="POST" action="{{ route('profile.update') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name', Auth::guard('participant')->user()->name) }}" 
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" value="{{ old('email', Auth::guard('participant')->user()->email) }}" 
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" required>
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="phone" id="phone" value="{{ old('phone', Auth::guard('participant')->user()->phone) }}" 
                                ...
                                <div>
                                    ...
                                    <input type="text" name="address" id="address" value="{{ old('address', Auth::guard('participant')->user()->address) }}" 
                                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
