            ...
            <div class="lg:col-span-3 space-y-6">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
                    <div class="relative z-10">
                        ...
                        <div class="flex space-x-3">
                            <a href="{{ route('home') }}#kegiatan" class="bg-white text-blue-800 px-5 py-2 rounded-lg font-semibold text-sm hover:bg-blue-50 transition shadow-sm">
                                Cari Program Baru
                            </a>
                            <a href="{{ route('user.history') }}" class="bg-blue-700 border border-blue-500 text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-blue-600 transition shadow-sm">
                                Semak Derma
                            </a>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition">
                        ...
                        <div class="text-2xl font-bold text-gray-800">
                            {{-- Kira program peserta, kalau belum ada logic letak 0 --}}
                            {{ Auth::guard('participant')->user()->registrations->count() ?? 0 }}
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition">
                        ...
                        <div class="text-2xl font-bold text-gray-800">
                            {{-- Kira total derma, kalau belum ada relation 'donations' letak RM 0 --}}
                            RM {{ number_format(Auth::guard('participant')->user()->donations->sum('amount') ?? 0, 2) }}
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                        ...
                        <a href="{{ route('participant.registrations') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">Lihat Semua</a>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse(Auth::guard('participant')->user()->registrations()->latest()->take(3)->get() as $reg)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 font-medium text-gray-900">
                                        {{ $reg->event->title ?? 'Program Tidak Dijumpai' }}
                                    </td>
