        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                ...
                <div class="bg-blue-100 text-blue-800 px-4 py-2 rounded-lg font-bold">
                    Total: RM {{ number_format($donations->sum('amount'), 2) }}
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 font-mono font-medium text-gray-900">
                                #{{ $donation->transaction_id ?? $donation->id }}
                            </td>
                            <td class="px-6 py-4 text-gray-600">
                                <div>{{ \Carbon\Carbon::parse($donation->created_at)->format('d M Y') }}</div>
                                <div class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($donation->created_at)->format('h:i A') }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($donation->status == 'success' || $donation->status == 'berjaya')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                ...
                                <a href="{{ url('/') }}#donasi" class="text-blue-600 hover:underline mt-2 inline-block">Mula menderma sekarang</a>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
