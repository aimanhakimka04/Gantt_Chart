...
<body class="bg-gray-50">
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex space-x-6 items-center">
                    ...
                    <div class="relative ml-2 pl-4 border-l border-blue-400">
                        ...
                        @if(Auth::guard('participant')->check())
                            <button id="user-menu-btn" onclick="toggleUserMenu()" class="flex items-center space-x-2 bg-blue-900 bg-opacity-40 hover:bg-opacity-60 border border-blue-400 text-white px-3 py-2 rounded-lg transition-all">
                                <div class="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-xs font-bold uppercase">
                                    {{-- Ambil huruf pertama nama participant --}}
                            ...
                            <div id="user-menu-dropdown" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden text-gray-800">
                                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <p class="text-sm font-bold truncate">{{ Auth::guard('participant')->user()->email }}</p>
                                </div>
                                
                                {{-- MENU ITEM: DASHBOARD (Jika ada) --}}
                                <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-sm hover:bg-blue-50 hover:text-blue-700">
                                    <i class="fas fa-home mr-2"></i> Dashboard Saya
                                </a>

                                {{-- MENU ITEM: SEJARAH DERMA --}}
                                <a href="{{ route('user.history') }}" class="block px-4 py-2 text-sm hover:bg-blue-50 hover:text-blue-700">
                                    <i class="fas fa-history mr-2"></i> Sejarah Derma
                                </a>

                                <div class="border-t border-gray-100 my-1"></div>

                                {{-- LOGOUT FORM FOR PARTICIPANT --}}
                        ...
                        {{-- CHECK IF ADMIN (COMMITTEE) IS LOGGED IN --}}
                        @elseif(Auth::guard('committee')->check())
                            ...
                            <div id="user-menu-dropdown" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden text-gray-800">
                                <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-sm hover:bg-purple-50 hover:text-purple-700">
                                    Dashboard Admin
                                </a>
                        ...
                        @else
                            <div class="flex space-x-2">
                                <a href="{{ route('login') }}" class="flex items-center space-x-1 bg-white text-blue-800 px-4 py-2 rounded-lg font-bold hover:bg-blue-50 transition-colors shadow-sm text-sm">
                                    <span>Log Masuk</span>
                                </a>
                            </div>
                        @endif
    ...
    <section id="kegiatan" class="py-16 bg-white">
                    ...
                    @foreach($events as $event)
                        <div class="bg-gray-50 rounded-xl p-6 hover:shadow-lg transition-shadow border border-gray-100 flex flex-col h-full">
                            <div class="flex items-start justify-between mb-4">
                                ...
                                @if($event->capacity)
                                    <span class="text-xs font-semibold px-2 py-1 bg-green-100 text-green-700 rounded-full">
                                        Terhad: {{ $event->capacity }} Orang
                                    </span>
                                @endif
                            </div>

                            <div class="flex-grow">
                                <h4 class="text-xl font-bold text-gray-900 mb-2 line-clamp-2">{{ $event->title }}</h4>
                                ...
                                <div class="space-y-2 text-sm text-gray-500 mb-6">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="truncate">{{ $event->venue ?? 'Masjid MMU' }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-auto pt-4 border-t border-gray-200">
                                    ...
                                    @if($isRegistered)
                                        <button disabled class="w-full py-2 px-4 bg-gray-100 text-gray-500 font-semibold rounded-lg cursor-not-allowed flex items-center justify-center">
                                            ...
                                            @if($isRegistered->status == 'pending')
                                                Menunggu Kelulusan
                                            @elseif($isRegistered->status == 'approved')
                                                Sudah Didaftar
                                            @else
                                                Permohonan Ditolak
                                            @endif
                                ...
                                @elseif(Auth::guard('committee')->check())
                                    <a href="{{ route('admin.events.index') }}" class="block w-full py-2 px-4 bg-purple-600 hover:bg-purple-700 text-white text-center font-semibold rounded-lg transition-colors">
                                        Urus Acara (Admin)
                                    </a>
                                @else
                                    <a href="{{ route('login') }}" class="block w-full py-2 px-4 bg-white border-2 border-blue-600 text-blue-600 hover:bg-blue-50 text-center font-semibold rounded-lg transition-colors">
        ...
        <div class="fixed top-20 right-5 z-50 animate-bounce">
            <div class="{{ session('success') ? 'bg-green-100 border-green-500 text-green-700' : 'bg-red-100 border-red-500 text-red-700' }} border-l-4 p-4 rounded shadow-lg flex items-center" role="alert">
                <p class="font-bold mr-2">{{ session('success') ? 'Berjaya!' : 'Ralat!' }}</p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div>
                    ...
                    <div class="space-y-6">
                        @if(isset($donationPrograms) && $donationPrograms->count() > 0)
                            @foreach($donationPrograms as $program)
                            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                <div class="flex justify-between items-center mb-3">
                                    <h5 class="text-lg font-bold text-gray-800">{{ $program->title }}</h5>
                                    <span class="text-blue-600 font-bold bg-blue-50 px-2 py-1 rounded text-sm">{{ $program->percentage }}%</span>
                                </div>
                                
                                <div class="w-full bg-gray-100 rounded-full h-3 mb-3 overflow-hidden">
                                    <div class="bg-blue-600 h-3 rounded-full transition-all duration-1000 ease-out" style="width: {{ $program->percentage }}%"></div>
                                </div>
                                
                                <div class="flex justify-between text-sm text-gray-600 font-medium">
                                    <span class="flex items-center text-green-600">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                        RM {{ number_format($program->current_amount, 2) }}
                                    </span>
                                    <span class="text-gray-400">Sasaran: RM {{ number_format($program->target_amount, 0) }}</span>
                                </div>
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <input type="text" id="donor-name" 
                                    ...
                                    value="{{ Auth::guard('participant')->check() ? Auth::guard('participant')->user()->name : '' }}">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="email" id="donor-email" 
                                    ...
                                    value="{{ Auth::guard('participant')->check() ? Auth::guard('participant')->user()->email : '' }}">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="tel" id="donor-phone" 
                                    ...
                                    value="{{ Auth::guard('participant')->check() ? Auth::guard('participant')->user()->phone : '' }}">
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <select id="donation-program" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors appearance-none bg-white">
                                        ...
                                        @if(isset($donationPrograms))
                                            @foreach($donationPrograms as $program)
                                                {{-- PENTING: Tukar value="{{ $program->title }}" kepada value="{{ $program->id }}" --}}
                                                <option value="{{ $program->id }}">{{ $program->title }}</option>
                                            @endforeach
                                        @endif
                                        
                                        {{-- Kekalkan value string untuk tabung umum --}}
            ...
            if (donationForm) {
                donationForm.addEventListener('submit', function(e) {
                    ...
                    const payload = {
                        ...
                        type: program,
                        _token: '{{ csrf_token() }}' // Important for Laravel security
                    };
                    ...
                    .then(data => {
                        ...
                        showDonationSuccess({
                            ...
                            donorName: "{{ Auth::guard('participant')->check() ? Auth::guard('participant')->user()->name : 'Hamba Allah' }}"
                        });
